...
const NON_PRODUCTIVE_DISPLAY_STATES = [...new Set([...NON_PRODUCTIVE_STATES])];
...
function getAttendanceAnalyticsByPeriod(granularity, periodId, agentFilter, policyOptions) {
  return rpc('getAttendanceAnalyticsByPeriod', () => {
    ...
    userDayMetrics.forEach(metrics => {
      ...
      totalBreakCreditSecs += 0;
      totalLunchAdjustmentSecs += 0;

      const compliance = userComplianceMap.get(metrics.user);
      if (compliance) {
        compliance.breakCreditSecs += 0;
        compliance.lunchAdjustmentSecs += 0;
        if (metrics.isWeekend) {
          compliance.breakCreditWeekendSecs += 0;
          compliance.lunchAdjustmentWeekendSecs += 0;
          compliance.adjustedWeekendSecs += adjustedTotal;
          compliance.weekendBaseCappedSecs += baseCapped;
        } else {
          compliance.breakCreditWeekdaySecs += 0;
          compliance.lunchAdjustmentWeekdaySecs += 0;
    ...
    const totalNonProductiveSecs = NON_PRODUCTIVE_STATES.reduce(
      (sum, state) => sum + (stateDuration[state] || 0),
      0
    );
    const totalNonProductiveHours = Math.round((totalNonProductiveSecs / 3600) * 100) / 100;
...
function calculateUserCompliance(filtered) {
    ...
    filtered.forEach(r => {
        if (!userStats.has(r.user)) {
            userStats.set(r.user, {
                ...
                breakSecs: 0,
                lunchSecs: 0,
                weekdayDeductionSecs: 0,
                weekendDeductionSecs: 0
            });
        }
    ...
    dailyBuckets.forEach((bucket, key) => {
        ...
        if (isWeekend) {
            stats.weekendDeductionSecs += deduction;
        } else {
            stats.weekdayDeductionSecs += deduction;
        }
    });

    return Array.from(userStats.entries()).map(([user, stats]) => {
        const availableWeekdaySecs = Math.max(0, stats.weekdayProdSecs - stats.weekdayDeductionSecs);
        const availableWeekendSecs = Math.max(0, stats.weekendProdSecs - stats.weekendDeductionSecs);
        const baseBillableSecs = Math.max(0, availableWeekdaySecs + availableWeekendSecs);
        const adjustedBillableSecs = baseBillableSecs;
        return {
            user,
...
function calculateBreakCreditSecs() {
  return 0;
}

function calculateLunchAdjustmentSecs() {
  return 0;
}
...
function calculateBreakLunchDeductions(breakSeconds, lunchSeconds) {
  const safeBreak = Number.isFinite(breakSeconds) ? breakSeconds : 0;
  const safeLunch = Number.isFinite(lunchSeconds) ? lunchSeconds : 0;

  const breakOver = Math.max(0, safeBreak - DAILY_BREAKS_SECS);
  const lunchOver = Math.max(0, safeLunch - DAILY_LUNCH_SECS);
  const combinedOver = Math.max(0, (safeBreak + safeLunch) - (DAILY_BREAKS_SECS + DAILY_LUNCH_SECS));
...
function generateDailyPivotMatrix(filteredRows, granularity, periodValue, options = {}) {
  ...
  const userData = allUsers.map(user => {
    ...
    const dailyData = dateRange.map(dateInfo => {
      ...
      const breakCreditHours = calculateBreakCreditSecs(breakMin * 60) / 3600;
      ...
      const formattedHours = adjustedHours.toFixed(2);
      const isLow = performanceStatus === 'under';

      return {
        date: dateInfo.date,
        value: formattedHours,
        numericValue: adjustedHours,
        rawValue: rawHours,
        breakCreditHours,
        isStatus: false,
        isLow: isLow,
...
function exportAdherenceComplianceSheet(payload) {
  return rpc('exportAdherenceComplianceSheet', () => {
    ...
    try {
      ...
      const cursor = new Date(exportData.startDate.getTime());
      const endMs = exportData.endDate.getTime();
      while (cursor.getTime() <= endMs) {
        const dateKey = Utilities.formatDate(cursor, timezone, 'yyyy-MM-dd');
        const display = Utilities.formatDate(cursor, timezone, 'MMM d, yyyy');
        const weekday = Utilities.formatDate(cursor, timezone, 'EEEE');
        dateList.push({ dateKey, display, weekday });
        dateKeysSet.add(dateKey);
        cursor.setDate(cursor.getDate() + 1);
      }

      if (!dateList.length) {
        return { success: false, error: 'No dates found in the selected range.' };
      }

      const agentsMap = new Map();
      exportData.dailyRows.forEach(row => {
        if (!row || !row.dateKey || !dateKeysSet.has(row.dateKey)) return;
        ...
        const target = agentsMap.get(key);
        const percent = Number(row.adherencePercent) || 0;
        target.dayMap.set(row.dateKey, {
          percent,
          breakMinutes: Number(row.breakMinutes) || 0,
          lunchMinutes: Number(row.lunchMinutes) || 0,
          overMinutes: Number(row.overTotalMinutes || 0) || 0
        });
        target.totalPercent += percent;
        target.eligibleDays += 1;
      });

      const agents = Array.from(agentsMap.values()).sort((a, b) => (a.agentName || '').localeCompare(b.agentName || ''));

      const weekdayHeaders = ['Agent', ...dateList.map(d => d.weekday), 'Weekly Adherence %', 'Compliance Status'];
      const dateHeaders = ['Date', ...dateList.map(d => d.display), '', ''];

      sheet.getRange(1, 1, 1, 1).setValue('Daily Adherence Matrix').setFontWeight('bold').setFontSize(14);
      sheet.getRange(2, 1, 1, Math.max(weekdayHeaders.length, 2)).setValues([[
        `Generated: ${Utilities.formatDate(now, timezone, 'yyyy-MM-dd HH:mm')}`,
        `Period: ${periodLabel}`
      ]]);

      sheet.getRange(3, 1, 1, weekdayHeaders.length)
        .setValues([weekdayHeaders])
        .setFontWeight('bold')
        .setBackground('#111827')
        .setFontColor('#ffffff');

      sheet.getRange(4, 1, 1, weekdayHeaders.length)
      ...
      const dataRows = agents.map(agent => {
        ...
        dateList.forEach(dateEntry => {
          const detail = agent.dayMap.get(dateEntry.dateKey);
          dayDetails.push(detail || null);
          values.push(detail ? Number(detail.percent) : '');
        });
        const weeklyPercent = agent.eligibleDays > 0
          ? Math.round((agent.totalPercent / agent.eligibleDays) * 10) / 10
          : 0;
        const complianceStatus = agent.eligibleDays > 0
          ? (weeklyPercent >= 95 ? 'Pass' : 'Fail')
          : 'N/A';
        values.push(weeklyPercent);
        values.push(complianceStatus);
        return { values, dayDetails, weeklyPercent, eligibleDays: agent.eligibleDays };
      });

      if (dataRows.length) {
        const values = dataRows.map(r => r.values);
        sheet.getRange(currentRow, 1, values.length, weekdayHeaders.length).setValues(values);
        if (dateList.length > 0) {
          sheet.getRange(currentRow, 2, values.length, dateList.length).setNumberFormat('0.0"%"');
        }
        sheet.getRange(currentRow, 1 + dateList.length + 1, values.length, 1).setNumberFormat('0.0"%"');

        const backgrounds = [];
        const fontColors = [];
        dataRows.forEach(row => {
          const bgRow = ['#ffffff'];
          const fontRow = ['#0f172a'];
          row.dayDetails.forEach(detail => {
            ...
            bgRow.push(adherenceColorForPercent(percent));
            fontRow.push(adherenceFontForPercent(percent));
          });
          bgRow.push(adherenceColorForPercent(row.weeklyPercent));
          fontRow.push(adherenceFontForPercent(row.weeklyPercent));
          bgRow.push(row.weeklyPercent >= 95 && row.eligibleDays > 0 ? '#22c55e' : '#ef4444');
          fontRow.push('#ffffff');
          backgrounds.push(bgRow);
          fontColors.push(fontRow);
        });

        sheet.getRange(currentRow, 1, backgrounds.length, weekdayHeaders.length)
      ...
      dateList.forEach((_, idx) => {
        const values = [];
        dataRows.forEach(row => {
          const detail = row.dayDetails[idx];
          if (detail && Number.isFinite(detail.percent)) {
            values.push(Number(detail.percent));
          }
        });
        if (!values.length) {
          averageRow.push('');
          ...
          averageRow.push(Math.round(avg * 10) / 10);
        }
      });

      const weeklyPercents = dataRows
        .filter(r => r.eligibleDays > 0)
        .map(r => Number(r.weeklyPercent) || 0);
      const weeklyAverage = weeklyPercents.length
        ? Math.round((weeklyPercents.reduce((sum, v) => sum + v, 0) / weeklyPercents.length) * 10) / 10
        : 0;
      const complianceRate = dataRows.length
        ? Math.round(((dataRows.filter(r => r.weeklyPercent >= 95 && r.eligibleDays > 0).length / dataRows.length) * 100) * 10) / 10
        : 0;

      averageRow.push(weeklyAverage);
      averageRow.push(`Pass Rate ${complianceRate}%`);

      sheet.getRange(currentRow, 1, 1, weekdayHeaders.length)
        .setValues([averageRow])
        .setFontWeight('bold')
        .setBackground('#f1f5f9');

      const avgBackgrounds = [[]];
      const avgFonts = [[]];
      averageRow.forEach((value, idx) => {
        if (idx === 0 || idx >= weekdayHeaders.length - 1) {
          avgBackgrounds[0].push('#f1f5f9');
          avgFonts[0].push('#0f172a');
          return;
        }
        const numeric = Number(value);
        const bg = adherenceColorForPercent(Number.isFinite(numeric) ? numeric : null);
        avgBackgrounds[0].push(bg);
        avgFonts[0].push(adherenceFontForPercent(Number.isFinite(numeric) ? numeric : null));
      });
      ...
      if (dateList.length) {
        sheet.getRange(currentRow, 2, 1, dateList.length).setNumberFormat('0.0"%"');
      }
      sheet.getRange(currentRow, 1 + dateList.length + 1, 1, 1).setNumberFormat('0.0"%"');

      sheet.setFrozenRows(4);
      sheet.setFrozenColumns(1);
      sheet.autoResizeColumns(1, weekdayHeaders.length);
      currentRow += 2;
      ...
      return {
        ...
        sheetTitle,
        periodLabel,
        userScope: exportData.userScopeLabel || 'All Users',
...
function loadBreakLunchAdherenceDaily_(startDate, endDate, userSet, timezone) {
  let allRows;
  try {
    allRows = fetchAllAttendanceRows();
  } catch (attendanceError) {
    console.warn('Unable to load attendance rows for adherence export:', attendanceError);
    return [];
  }
  ...
  for (let i = 0; i < allRows.length; i++) {
    ...
    const comparableMs = ensureComparableMs(row);
    if (!Number.isFinite(comparableMs)) continue;
    if (comparableMs < startMs || comparableMs > endMs) continue;

    const user = String(row.user || row.User || row.employee || '').trim();
    if (!user) continue;
    const userKey = user.toLowerCase();
    if (userSet.size > 0 && !userSet.has(userKey)) continue;
    ...
    if (!entries.has(entryKey)) {
      ...
      entries.set(entryKey, {
        ...
        weekStartKey,
        agentName: user,
        agentId: row.userId || row.UserId || row.UserID || '',
        breakMinutes: 0,
        lunchMinutes: 0
      });
    }
  ...
  return Array.from(entries.values()).map(entry => {
    ...
    return {
      ...entry,
      breakMinutes: Math.round(entry.breakMinutes * 100) / 100,
...
function summarizeOverallBreakLunch_(agentSummaries) {
  if (!Array.isArray(agentSummaries) || agentSummaries.length === 0) {
    return {
      ...
      totalEligibleDays: 0,
      totalAdherentDays: 0,
      totalNonAdherentDays: 0,
      totalOverMinutes: 0
    };
  }

  const totals = agentSummaries.reduce((acc, entry) => {
    acc.totalEligibleDays += entry.eligibleDays;
    acc.totalAdherentDays += entry.adherentDays;
    acc.totalNonAdherentDays += entry.nonAdherentDays;
    acc.totalOverMinutes += entry.totalOverMinutes;
    return acc;
  }, {
    overallAdherencePercent: 0,
    totalEligibleDays: 0,
  ...
  totals.overallAdherencePercent = totals.totalEligibleDays > 0
    ? Math.round((totals.totalAdherentDays / totals.totalEligibleDays) * 1000) / 10
    : 0;
...
function createBasicAnalytics(filtered, granularity, periodId, agentFilter, periodStart, periodEnd, managerDirectory, hourPolicy) {
  ...
  fallbackDayMetrics.forEach(dayMetrics => {
    ...
    fallbackBreakCreditSecs += 0;
    fallbackLunchAdjustmentSecs += 0;
